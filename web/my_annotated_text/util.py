# Override to add custom padding around text
...
# Replaces `https://github.com/tvst/st-annotated-text/blob/master/annotated_text/util.py`
def annot(body, label="", background=None, color=None, **style):
    ...
    return span(style=styles(background=background, **color_style, **style,))(
        html.escape(body),
        span(style=styles(padding_left=rem(0.5), text_transform="uppercase",))(
            span(style=styles(font_size=em(0.67), opacity=0.5, color="#222"))(
                html.escape(label),
            ),
        ),
    )
